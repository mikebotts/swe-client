  ...
  var 
    ...
    sQuery.fn = sQuery.prototype = {
      ...
      interpretfeed: function (data, templateobject) {
        ...
        var dataType = templateobject.datatype;
        var ret = {};
        
        switch (dataType) {
          case "textDataRecord":
            var vals = data.trim().split(templateobject.tokenseparator);
            $.each(templateobject.tokens, function( index, token) {
      ...
      openwsfeed: function(feedsource, feedonmessage, feedonerror, feedonclose) {
        ...
        ws.onmessage = function (event) {
          if (feedonmessage) {
            var args = retArgs.slice();
            args[args.length] = event;
            feedonmessage.apply(null, args);
          }
        }
